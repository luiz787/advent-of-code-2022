...
fn main() {
    ...
    let result = starting_positions
        .iter()
        .map(|(sx, sy)| shortest_path(&adjacency_map, (*sx, *sy), end))
        .filter(|optional_cost| optional_cost.is_some())
        .map(|optional_cost| optional_cost.unwrap())
        .min()
        .unwrap();
